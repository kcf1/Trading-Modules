...
class Evaluator:
    ...
    ff3_reader = pdr.famafrench.FamaFrenchReader(
        "F-F_Research_Data_Factors_daily", start="1900-01-01", end=str(dt.today())
    )
    ff3 = ff3_reader.read()[0] / 100
    ff3_reader.close()

    mom_reader = pdr.famafrench.FamaFrenchReader(
        "F-F_Momentum_Factor_daily", start="1900-01-01", end=str(dt.today())
    )
    ...
    def get_summary(self):
        ...
        summary.loc["CAPM Alpha", ["Value", "Sample size", "P-value"]] = (
            self.get_CAPMAlpha()
        )
        summary.loc["Carhart Alpha", ["Value", "Sample size", "P-value"]] = (
            self.get_CarhartAlpha()
        )
        ...
        summary.loc["Exp Shortfall 95%", ["Value", "Sample size"]] = (
            self.get_expectedShortfall(0.95)
        )
        summary.loc["VaR 99%", ["Value", "Sample size"]] = self.get_VaR(0.99)
        summary.loc["Exp Shortfall 99%", ["Value", "Sample size"]] = (
            self.get_expectedShortfall(0.99)
        )
    ...
    def get_sharpeRatio(self, included_RF: bool = True):
        sharpeRatio = (
            self.fourFactor["Strategy-RF"].mean()
            / self.fourFactor["Strategy-RF"].std()
            * np.sqrt(252)
        )
        sampleSize = self.returns.size
        ...
        p_2sided = 2 * (1 - t.cdf(abs(t_stat), sampleSize))
