...
from ..indicators.technical_indicators import TechnicalIndicator


class DoubleRSI(TradingModel):
    ...
    def produce_rules(
        ...
        fast_rsi = ti.get_rsi(params["fast_rsi_lookback"])
        slow_rsi = ti.get_rsi(params["slow_rsi_lookback"])
        rsi_diff = fast_rsi - slow_rsi
        rsi_diff = rsi_diff / rsi_diff.abs().expanding(240).mean()
        ...
        rules["fast_rsi"] = fast_rsi
        rules["slow_rsi"] = slow_rsi
        rules["rsi_diff"] = fast_rsi - slow_rsi
        rules["ema"] = ema
    ...
    def produce_sides(
        ...
        long_entry = np.where(
            (rules.rsi_diff <= -3) & (rules.ema >= rules.ema_filter),
            -1,
            0,
        )
    ...
    def produce_sizes(
        ...
        if end_idx is None:
            self.bars.index[-1]
        sizes = pd.Series(index=self.sides.index)
        sizes.loc[:] = 1
