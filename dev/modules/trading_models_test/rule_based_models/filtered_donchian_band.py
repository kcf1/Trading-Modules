...
class FilteredDonchianBand(TradingModel):
    ...
    def produce_rules(
        ...
        upper, lower = ti.get_dc(
            params["channel_lookback"],
            params["channel_lookforward"],
            params["smooth_lookback"],
        )
    ...
    def produce_sides(
        ...
        long_entry = np.where(
            (rules.close - rules.lower <= 0.05 * rules.range)
            # & (rules.close - rules.lower >= -0.05 * rules.range)
            & (rules.close > rules.ema_filter),
            1,
            0,
        )
        short_entry = np.where(
            (rules.close - rules.lower >= 0.95 * rules.range)
            # & (rules.close - rules.lower <= 1.05 * rules.range)
            & (rules.close < rules.ema_filter),
            -1,
            0,
        )
